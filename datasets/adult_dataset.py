...
class AdultDataset(AbstractDataset):
    ...
    #FATTA MODIFICA: AGGIUNTI p_test e p_val COME IN GERMAN
    def __init__(self, split, args, normalize=True, p_test=0.2, p_val=0.2):
        super().__init__('adult', split, p_test, p_val)
        ...
        train_protected = torch.tensor(protected_train.astype(bool), device=self.device)
        ...
        self.protected_test = torch.tensor(protected_test.astype(bool), device=self.device)
