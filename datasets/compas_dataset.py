...
class CompasDataset(AbstractDataset):
    ...
    def __init__(self, split, args, p_test=0.2, p_val=0.2, discretize=True, preprocess=True):
        ...
        if not path.exists(datafile):
            request.urlretrieve(
                'https://raw.githubusercontent.com/propublica/compas-analysis/master/compas-scores-two-years.csv', datafile
            )
        ...
        X_train, self.X_test, y_train, self.y_test = train_test_split(
            features, labels, test_size=self.p_test, random_state=0
        )
        ...
        self.X_train, self.X_val, self.y_train, self.y_val = train_test_split(
            X_train, y_train, test_size=self.p_val, random_state=0
        )
